...
int main() {
  ...
  if (p_temp == NULL) {
    ...
    fprintf(
        stderr,
        "Error: realloc to larger size failed! Original buffer still valid.\n");
    ...
    // The *new* part (from index initial_size to larger_size-1) is
    // UNINITIALIZED. Let's initialize the new part
  ...
  if (p_temp == NULL) {
    ...
    fprintf(stderr, "Error: realloc to smaller size failed! Original buffer "
                    "still valid.\n");
  } else {
    p_char_buf = p_temp;
    printf("Resized to %zu bytes successfully at %p\n", smaller_size,
  ...
  if (p_realloc_as_malloc == NULL) {
    ...
    free(p_char_buf); // Free the potentially still valid original pointer if
                      // previous realloc(0) failed
    return 1;
  } else {
  ...
  printf("After memset(mem_buffer + 2, 0, 3): 'XX\0\0\0World!' (partial string "
         "due to embedded nulls)\n");
  // Printing character by character to show the nulls:
  printf("Char by char: ");
  for (size_t i = 0; i < 12; ++i) { // Print relevant part
    if (mem_buffer[i] == '\0')
      printf("[NULL]");
    else
  ...
  // **IMPORTANT:** Behavior is UNDEFINED if source and destination regions
  // overlap.
  ...
  memcpy(dest_data, src_data,
         strlen(src_data) + 1); // +1 to include null terminator
  ...
  // Example 1: Overlap, dest is after src (move_buffer+2 from move_buffer, 5
  // bytes) Effectively: "ab|cde|fghij" -> "ab|abcde|ij" (conceptually, 'cde'
  // shifts right) Should become "ab cde de ij" -> "ab cde cde ij" move_buffer +
  // 2 = cdefghij move_buffer     = abcdefghij copy from move_buffer (src) to
  // move_buffer + 2 (dest), 5 bytes target: move_buffer becomes "ababcdehij"
  ...
  // Example 2: Overlap, dest is before src (move_buffer from move_buffer+2, 5
  // bytes) Effectively: "ab|cde|fghij" -> "cde|fghij" (conceptually, 'cde'
  // shifts left) copy from move_buffer + 2 (src = "cdefg...") to move_buffer
  // (dest), 5 bytes target: move_buffer becomes "cdefgefghij" (no, just
  // "cdefghij")
  ...
  printf("1. Type Safety: malloc/calloc/realloc return void*, require explicit "
         "casts. `new` returns typed pointer.\n");
  printf("2. Initialization: malloc = garbage, calloc = zeros, `new` = calls "
         "constructor.\n");
  printf("3. Deallocation: free() just deallocates memory. `delete`/`delete[]` "
         "call destructors FIRST, then deallocate.\n");
  printf("4. Operators vs Functions: `new`/`delete` are operators (can be "
         "overloaded). malloc/free family are library functions.\n");
  printf("5. Array Size: `free()` doesn't know array size. `delete[]` usually "
         "does (implementation dependent).\n");
  printf("6. Error Handling: malloc family returns NULL on failure. `new` "
         "throws std::bad_alloc exception (by default).\n");
  printf("7. Zero Size: malloc(0) is implementation-defined (NULL or unique "
         "ptr). `new Type[0]` is allowed by standard.\n");
...
Feature          | C (malloc/calloc/realloc/free)       | C++
(new/delete/delete[])
-----------------|--------------------------------------|----------------------------------------
Return Type      | void* (needs cast)                   | Typed pointer (e.g.,
int*) Initialization   | malloc: None (garbage)               | Calls
constructor (default or custom) | calloc: Zero-initialized             |
Deallocation     | free(): Deallocates memory only       | Calls destructor,
then deallocates Type             | Library Functions                    |
Operators (overloadable) Arrays           | Must track size yourself for free()
| delete[] usually infers size Error Handling   | Returns NULL | Throws
std::bad_alloc exception Zero Size Alloc  | malloc(0): Implementation-defined |
new T[0]: Standard allows Type Safety      | Low (via void*) | High (typed)
Object Lifecycle | No constructor/destructor calls     | Automatic
